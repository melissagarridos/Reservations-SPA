...
export function renderDashboard() {
        ...
        reservationList.innerHTML = reservations.map(reservation => `
          <div class="flex items-center gap-3 md:gap-4 bg-white/[0.03] border border-white/[0.07]
               ...
               <div class="flex items-center gap-3 md:gap-4 bg-white/[0.03] border border-white/[0.07]
                      ...
                      <br>startHour-id="${reservation.startHour}"
                      <br>endHour-id="${reservation.endHour}"
                      <br>reason-id="${reservation.reason}">
    ...
    async function addreservation(title) {
    ...
    async function updatereservation(id, newTitle) {
      const res = await fetch(`${API}/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    reservationList.addEventListener("click", async (e) => {
      ...
      if (action === "toggle") {
        ...
        if (action === "save") {
            ...
            if (newTitle) await updatereservation(id, newTitle);
        }
    ...
    document.getElementById("add-reservation-btn").addEventListener("click", async () => {
        const input = document.getElementById("new-reservation-input");
        const title = input.value.trim();
        if (!title) return;
        input.value = "";
        await addreservation(title);
    });

    document.getElementById("new-reservation-input").addEventListener("keydown", async (e) => {
